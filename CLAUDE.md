...
- Web-served PHP: **8.4.24** as of rung 4b, deployed 2026-08-04 (8.3.33 before that, 7.4.33 originally). This is the baseline the Docker time capsule must match — set `PHP_VERSION=8.4` in `.env`.
- **OPcache is on** (enabled 2026-08-04, `validate_timestamps=On` / `revalidate_freq=2`, 128 MB, 10000 slots): deploys pick up changed files within seconds, so no pool restart is needed. Should `validate_timestamps` ever be set to 0, every deploy then requires `pkill -u ourlink -f php`. `/en/sm/cache-status` reports **APCu only** — OPcache is a separate segment and is not visible there.
