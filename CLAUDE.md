...
## Production environment (verified 2026-08-01 via SSH; PHP moved to 8.5 on 2026-08-11 and the server-side details below have not been re-read since)

- Web-served PHP: **8.5** as of 2026-08-11 (8.4.24 from rung 4b on 2026-08-04, 8.3.33 before that, 7.4.33 originally). The exact patch level has not been read off the server since the move — anything that turns on it needs `php -v` there first. The capsule serves 8.5.9, so it **matches production again** and reproducing a live bug locally no longer starts with a version switch. `config.platform.php` stays at **8.4.24** and is not a contradiction: it is the ceiling twelve locked packages impose, not a claim about any runtime, and raising it would force `--ignore-platform-req=php+` on every install. See [docs/php-85.md](docs/php-85.md).
...
- **ICU was 72.1 on the 8.4 build, down from 76.1 on the 8.3 build** (Unicode 15.0, CLDR-era TZData 2022e) — the hoster's newer PHP shipped the *older* ICU. **What the 8.5 build carries is unmeasured**, and it is the first thing to check on the server now, because the 8.3 → 8.4 move already proved the hoster's builds do not carry ICU forward monotonically. Nothing crashes either way, but 27 `IntlDateFormatter` call sites format dates against whatever it is, so a rendering difference is possible in both directions. PHP's own timezone database is separate and current (2026.3).
...
- `apc.shm_size` was **32M** with `apc.ttl=0` on the 8.4 ini, so the oversized-item-wipes-cache hazard persisted; whether the 8.5 ini inherited those values is unverified, and the konsoleH ticket now needs to name whichever ini the 8.5 build reads (it was `php84-ini`) — confirm the name before filing.
