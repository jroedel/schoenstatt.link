#!/bin/bash
shopt -s nullglob
for image_file in SL*A.jpg SL*A.JPG SL*A.jpeg SL*A.png SL*A-??.jpg SL*A-??.JPG SL*A-??.jpeg SL*A-??.png
do
filename="${image_file%.*}"
...
if [ ! -f $file180 ]; then
    echo "$image_file => $file180"
    gm convert $image_file -resize x180 $file180
fi
if [ ! -f $file400 ]; then
