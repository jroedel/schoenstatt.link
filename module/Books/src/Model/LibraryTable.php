    ...
    ProblemProviderInterface
{
    ...
    /**
     * How many consecutive renewals a checkout gets when a library has not said.
     * A library may override with lib_libraries.MaximumBookRenewals; 0 there turns
     * renewal off for that library.
     */
    const DEFAULT_MAXIMUM_RENEWALS = 3;
...
//                     'checkedInUserAgent'    => $row['CheckedInUserAgent']),
    ...
    /** Why a renewal was refused; the caller turns these into a message. */
    public const RENEW_OK                = 'ok';
    public const RENEW_NO_SUCH_CHECKOUT  = 'no-such-checkout';
    public const RENEW_ALREADY_RETURNED  = 'already-returned';
    public const RENEW_LIMIT_REACHED     = 'limit-reached';
    public const RENEW_DISABLED          = 'renewal-disabled';

    /**
     * Extend a checkout, and actually say so in the database.
     *
     * This replaces a 2020 stub that computed a date, returned it, and persisted
     * nothing — no UPDATE, no TimesRenewed, no LastRenewedOn, no limit. It had
     ...
     * is renewed to a full period from now, not to a date that already passed.
     *
     * The limit is consecutive renewals per checkout, from
     * lib_libraries.MaximumBookRenewals, defaulting to DEFAULT_MAXIMUM_RENEWALS.
     * A library setting 0 turns renewal off for itself.
     *
     * @param int $checkoutId
     * @return array{status: string, dueOn: ?\DateTimeInterface, timesRenewed: int, maximum: int}
     */
    public function renewBook($checkoutId)
    {
        $checkout = $this->getCheckout((int) $checkoutId);
        if (! isset($checkout['checkoutId'])) {
            return ['status' => self::RENEW_NO_SUCH_CHECKOUT, 'dueOn' => null, 'timesRenewed' => 0, 'maximum' => 0];
        }
        //A returned book is not renewable, and the check must come before the limit
        //so the reader is told the true reason rather than a plausible one.
        if (isset($checkout['checkedInOn']) && null !== $checkout['checkedInOn']) {
            return ['status' => self::RENEW_ALREADY_RETURNED, 'dueOn' => null, 'timesRenewed' => 0, 'maximum' => 0];
        }
        ...
        $maximum = (int) $libraryOptions->maximumBookRenewals;
        $timesRenewed = isset($checkout['timesRenewed']) ? (int) $checkout['timesRenewed'] : 0;
        if (0 === $maximum) {
            return [
                'status' => self::RENEW_DISABLED,
                'dueOn' => null,
                'timesRenewed' => $timesRenewed,
                'maximum' => $maximum,
            ];
        }
        if ($timesRenewed >= $maximum) {
            return [
                'status' => self::RENEW_LIMIT_REACHED,
                'dueOn' => null,
                'timesRenewed' => $timesRenewed,
                'maximum' => $maximum,
            ];
        ...
        $gateway->update([
            'DueOn'         => $newDueOn->format('Y-m-d H:i:s'),
            'TimesRenewed'  => $timesRenewed + 1,
            'LastRenewedOn' => (new \DateTime('now', new \DateTimeZone('UTC')))->format('Y-m-d H:i:s'),
        ], ['CheckoutId' => (int) $checkoutId]);

        $this->removeDependentCacheItems('checkout');

        return [
            'status'       => self::RENEW_OK,
            'dueOn'        => $newDueOn,
            'timesRenewed' => $timesRenewed + 1,
            'maximum'      => $maximum,
        ];
...
//         }
    ...
    protected function processLibraryRow($row)
    {
        ...
        $processedRow = [
            ...
            'maximumBookRenewals'   => $this->filterDbInt($row['MaximumBookRenewals']),
