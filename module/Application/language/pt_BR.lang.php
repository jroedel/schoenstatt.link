<?php

return array(
    'About half of the work to maintain a reliable, centralized source of information depends on programmers who can provide the easy-to-use tools to submit and access the data.' => ' Quase a metade do trabalho em manter uma fonte de informação confiável e centralizada depende de programadores que possam disponibilizar ferramentas intuitivas para inserir, editar e consultar conteúdo.',
    'Are you a developer?' => 'É um programador?',
    'Are you a Kentenich scholar?' => 'É um especialista em Kentenich?',
    'Are you bilingual?' => 'É bilíngue?',
    'Contact us' => 'Fale conosco',
    'Email' => 'E-mail',
    'Google Knowledge Graph' => 'Google Knowledge Graph',
    'Learn more' => 'Saiba mais',
    'Probably the greatest challenge for our generation is transmitting the wisdom and charism of our Father and Founder. Help us by completing our bibliography of Fr. Kentenich and writing brief book summaries.' => 'Provavelmente, o maior desafio para nossa geração é transmitir a sabedoria e carisma do nosso Fundador. Ajude-nos completando nossa bibliografia do Padre Kentenich e Schoenstatt em todos os idiomas e escrevendo introduções e sumários dos livros.',
    'Schoenstatt is an international movement which requires to provide information in many languages. Help us make our site accessible to the whole world!' => ' Schoenstatt é um movimento internacional que quer oferecer informação em vários idiomas. Ajude-nos a fazer nosso site acessível para o mundo inteiro!',
    'Schoenstatt Link' => 'Schoenstatt Link',
    'Schoenstatt Link hopes to facilitate communication by dramatically increasing and improving Schoenstatt\'s web presence. We use the %s to help internet users find the Schoenstatt communities and shrines nearby, as well information on Schoenstatt books and material. Please help us centralize this information by the end of 2018!' => ' Schoenstatt Link espera facilitar a comunicação aumentando e melhorando a presença web de Schoenstatt. Utilizamos %s para ajudar os usuários que navegam na internet à encontrar comunidades schoenstatteanas e Santuários próximos, bem como informações de livros e materiais de Schoenstatt. Por favor, ajude-nos a centralizar essa informação até o final de 2018!',
    'Search contacts' => 'Pesquisar contatos',
    'Search publications' => 'Pesquisar publicações',
    'Sign in' => 'Entrar',
    'translation edits' => 'editar tradução',
    'Username' => 'Usuário',
    'We need your help!' => 'Precisamos da sua ajuda!',
);