...
| [timeline-and-corpus.md](timeline-and-corpus.md) | the Fr. Kentenich timeline and the Schmiedl text corpus: what the 527 events and 2,753 texts actually contain, why the link between them was never made, the design decisions already settled (one table with a `kind` discriminator, a single FK, an importer rather than a form), and the two-part plan — what to fix now, and what waits for laminas to be gone |
