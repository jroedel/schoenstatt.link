...
| A schema change | `database/db<N>.sql` | the `@phase`, `@kind`, `@tables` and `@verify` headers; [DEPLOY.md](DEPLOY.md) | rehearse through `tools/migrate.sh`, never by piping into `mysql`. `@verify` must select what is still **wrong** and return zero rows on success. Then `./tools/schema-dump.sh` and commit the diff — `ci-local.sh schema` fails while the recording is stale |
...
| The SQL builder itself | `SionModel\Db\Sql\*` — `Select`, `Insert`, `Update`, `Delete`, `Where` and the predicates | the rendered statement **and** the values its placeholders bind, in order | `ci-local.sh unit` (`SqlBuilderSurfaceTest`, no database needed) against `test/Db/sql-builder-surface.php`; then `./tools/sql-surface.sh --check` |
...
| A dependency | `composer.json` | `tools/dependency-audit.php` (`--laminas` for that scope); [laminas-exit.md](laminas-exit.md) §8 | `ci-local.sh composer`, and read the `composer.lock` diff package by package. Removing a package silently drops the undeclared ones it pulled in, and a declared-but-unused line may be a version pin |
