...
- **The container is ours** (2026-09-21). `App\Services\Container` — PSR-11, five keys:
  `services`, `factories`, `invokables`, `aliases`, `delegators`, shared by default, with
  `build()` for a fresh instance. That is the whole of what the merged configuration and
...
- **laminas does nothing here any more.** The last package, `laminas-stdlib`, went on
  2026-09-22 with laminas-db: `SionModel\Data\ArrayMerge` is the merge rule, `str_pad`
  replaced a copy of `AbstractStringWrapper::strPad()`, and the two `ArraySerializableInterface`
  implementations named an interface nothing type-hinted. laminas-i18n went at step 3,
  laminas-view at step 4, the whole form stack at iteration A, and the module system, the
  session, the container and the translator interface on 2026-09-21.
...
**No** `laminas/*` package is installed, down from 37. The last two that were transitive,
`config` and `loader`, left on 2026-09-21 with `laminas-modulemanager`; `eventmanager`
followed the same day with `laminas-session`, which was the only thing that had ever wanted
one; and `db` and `stdlib`, the last two, were direct lines of ours and went on 2026-09-22.
...
inputfilter, router, session, text, validator, view. All ten went, the last of them
`laminas-session` on 2026-09-21, leaving only our own direct `^3.24` line — and that left
the same day with the container, so `why-not` has nothing to answer any more. The last
measurement it gave, before the removal, is the one worth keeping:
...
`eventmanager` falling out behind them), and laminas-db and laminas-stdlib on 2026-09-22
to **0**. `stdlib` did not fall out behind `db`: both were direct lines of ours, so both
had to be removed by hand, and `JUser`'s own `composer.json` declared two more
(`laminas-http`, `laminas-permissions-acl`) that nothing had used since step 6.
...
| 8 ✅ | laminas-db, and `stdlib` after it | db 99 files, `SionTable` 2,412 lines over `TableGateway`/`Sql`; stdlib 5 live uses | **done 2026-09-22.** `SionModel\Db\*`, a thin PDO wrapper of ours (§6), gated on `test/Db/sql-surface.txt` — MariaDB's general log across a smoke run, the integration suite, the console commands and `test/Db/drive-tables.php`, 177 statement shapes, unchanged across the cutover. `stdlib` followed in the same iteration: the survey said nine references across nine files and four of them were docblock prose. `laminas-translator` was grouped here and left early, on 2026-09-21 |
...
Nothing. Step 8 closed on 2026-09-22 and **no `laminas/*` package is installed**. The
numbering above is the order the work was *planned* in, which is not the order it was
released in; the releases are the three iterations.
...
### Per-request laminas bridging (until step 8)

`ServiceBridge` builds the laminas-side container lazily and never bootstraps it. Two things
