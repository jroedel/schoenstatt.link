...
| --- | --- | --- | --- | --- |
| `opcache.interned_strings_buffer` | **`32`** | `32` | SYSTEM | **Landed — confirmed 2026-08-21** at `internedBufferConfiguredMb: 32`, 39.9% used, 68,328 strings. It had sat at 89–99% of 8 MiB (77,504 strings), and once full OPcache stops interning and stores duplicate strings per script. The confirming reading also settles that the old ceiling was really truncating: **13.4 MB in use on a freshly reset cache exceeds the entire 8 MiB buffer**. Nothing to do with whether the app fits — that is `memory_consumption`, at 24.6% |
