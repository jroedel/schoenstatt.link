...
# Check for the remote directory before touching anything. A missing
# data/exceptions/ means the recorder has never fired — that is the
# healthy case, not an error.
if ! DIR_CHECK=$(ssh -p "$SSH_PORT" "$REMOTE" \
    "[ -d '$REMOTE_EXC_DIR' ] && echo EXISTS || echo MISSING" 2>&1); then
...
case "$DIR_CHECK" in
    *MISSING*)
        echo "No $REMOTE_EXC_DIR on $REMOTE — no exceptions have been recorded. Nothing to fetch."
        exit 0
        ;;
    *EXISTS*)
        ;;
